...
def test_list_psbt_drafts_handles_missing_session_and_filters_invalid(mocker):
    ...
    assert BroadcastTransactionService.list_psbt_drafts(True) == []

    # Path 2: Valid session with mixed rows
    mock_session = MagicMock()
    mock_session.list_psbt.return_value = [
        {'id': 'valid_id', 'psbt': 'valid_psbt', 'signed': 1, 'purpose': 'send_asset'},
        'invalid_row_type',
...
def test_selector_titles_formats_correctly():
    ...
    items = [
        PsbtDraftItem(id='1234567890123', psbt='x', signed=False, purpose='send_asset'),
...
def test_parse_psbt_input_normalization():
    ...
    # Prefix with purpose
    p2 = BroadcastTransactionService.parse_psbt_input('psbt:send_asset:cHNi...')
...
def test_resolve_purpose_precedence():
    ...
    assert BroadcastTransactionService.resolve_purpose(PsbtParsed(psbt='', purpose='parsed_purpose'), 'selector') == 'parsed_purpose'
    assert BroadcastTransactionService.resolve_purpose(PsbtParsed(psbt='', purpose=None), 'selector') == 'selector'
    assert BroadcastTransactionService.resolve_purpose(PsbtParsed(psbt='', purpose=None), None) is None
...
def test_action_key_resolves_correct_mode():
    ...
    assert BroadcastTransactionService.action_key(False, 'send_asset') == 'sign'
    assert BroadcastTransactionService.action_key(True, 'send_btc') == 'send_btc'
    assert BroadcastTransactionService.action_key(True, 'send_asset') == 'send_asset'
    assert BroadcastTransactionService.action_key(True, 'inflate_asset') == 'inflation'
    assert BroadcastTransactionService.action_key(True, 'inflation') == 'inflation'
    assert BroadcastTransactionService.action_key(True, 'random_purpose') == 'create_utxos'
...
def test_receive_page_name_for_signed_psbt(mocker):
    ...
    mock_get_session.return_value = None
    assert BroadcastTransactionService.receive_page_name_for_signed_psbt('psbt') == 'NIA page'
    ...
    assert BroadcastTransactionService.receive_page_name_for_signed_psbt('psbt1') == 'NIA page'
    assert BroadcastTransactionService.receive_page_name_for_signed_psbt('psbt2') == 'IFA secondary issuance'
    assert BroadcastTransactionService.receive_page_name_for_signed_psbt('psbt_unused') == 'NIA page'
...
def test_cleanup_secondary_draft_if_any_execution_paths(mocker):
    ...
    BroadcastTransactionService.cleanup_secondary_draft_if_any('psbt', 'inflate_asset')
    
    mock_session = MagicMock()
    mock_get_session.return_value = mock_session
    
    # Bypass non-inflation
    BroadcastTransactionService.cleanup_secondary_draft_if_any('psbt', 'send_asset')
    ...
    BroadcastTransactionService.cleanup_secondary_draft_if_any('my_psbt', 'inflate_asset')
    mock_session.delete_secondary_draft_by_psbt.assert_called_once_with('my_psbt')
    ...
    BroadcastTransactionService.cleanup_secondary_draft_if_any('psbt:inflation:my_psbt', None)
...
def test_multisig_pending_context_evaluates_initiator(mocker):
    ...
    mock_get_val = mocker.patch('src.data.service.broadcast_transaction_service.local_store.get_value', return_value='local_xpub')
    
    assert BroadcastTransactionService.multisig_pending_context(None) is None
    
    mock_info = MagicMock()
    mock_info.operation = None
    assert BroadcastTransactionService.multisig_pending_context(mock_info) is None
    
    mock_info.operation = MagicMock()
...
def test_match_pending_operation_psbt_comparison(mocker):
    ...
    mock_multi = mocker.patch('src.data.service.broadcast_transaction_service.BroadcastTransactionService.multisig_pending_context')
    
    # Null checks
    assert BroadcastTransactionService.match_pending_operation(None, 'psbt') is None
    assert BroadcastTransactionService.match_pending_operation(MagicMock(), '') is None
    assert BroadcastTransactionService.match_pending_operation_by_txid(None, 'txid') is None
    assert BroadcastTransactionService.match_pending_operation_by_txid(MagicMock(), '') is None
    ...
    assert BroadcastTransactionService.match_pending_operation(MagicMock(), 'psbt_mismatch') is None
    assert BroadcastTransactionService.match_pending_operation(MagicMock(), 'psbt:purpose:parsed_psbt') == ctx
    
    # by_txid blindly matches if context returns (transaction matching is handled upstream)
    assert BroadcastTransactionService.match_pending_operation_by_txid(MagicMock(), 'txid') == ctx


def test_global_pending_operation_state_lifecycle():
    """State should persist across the class variables securely."""
    BroadcastTransactionService.set_pending_operation_state('my_info', 'my_txid')
...
def test_operation_transfer_type_key_mappings():
    ...
    assert BroadcastTransactionService.operation_transfer_type_key(op) == 'inflate_asset'
    
    op.is_INFLATION_TO_REVIEW.return_value = False
    op.is_SEND_TO_REVIEW.return_value = True
    assert BroadcastTransactionService.operation_transfer_type_key(op) == 'send_asset'
    
    op.is_SEND_TO_REVIEW.return_value = False
    op.is_SEND_BTC_TO_REVIEW.return_value = True
    assert BroadcastTransactionService.operation_transfer_type_key(op) == 'send_btc'
    
    op.is_SEND_BTC_TO_REVIEW.return_value = False
    op.is_CREATE_UTXOS_TO_REVIEW.return_value = True
    assert BroadcastTransactionService.operation_transfer_type_key(op) == 'create_utxos'
...
def test_set_rgb_mode_for_purpose(mocker):
    ...
    mock_store = mocker.patch('src.data.service.broadcast_transaction_service.hardware_client_store')
...
def test_get_transfer_type_label_localization(mocker):
    ...
    mock_trans = mocker.patch('src.data.service.broadcast_transaction_service.QCoreApplication.translate')
    ...
    assert BroadcastTransactionService.get_transfer_type_label('btc_transfer') == 'Translated String'
    
    # Fallback checks (when translate returns identical key)
    mock_trans.return_value = 'btc_transfer'
    assert BroadcastTransactionService.get_transfer_type_label('btc_transfer') == 'BTC transfer'
    
    mock_trans.return_value = 'unknown_key'
    assert BroadcastTransactionService.get_transfer_type_label('unknown_key') == 'unknown_key'
...
def test_rgb_transfer_inspection_summary_extraction():
    ...
    res_none = BroadcastTransactionService.rgb_transfer_inspection_summary(None, 'preset_key')
    ...
    # Test send logic
    summary_send = BroadcastTransactionService.rgb_transfer_inspection_summary(rgb_details, None)
    ...
    summary_inflate = BroadcastTransactionService.rgb_transfer_inspection_summary(rgb_details, None)
    assert summary_inflate.amount == 200
    assert summary_inflate.transfer_type_key == 'inflation'
    
    # Test exception suppression
    rgb_details.operations = property(lambda: (_ for _ in ()).throw(Exception('boom')))
    summary_err = BroadcastTransactionService.rgb_transfer_inspection_summary(rgb_details, None)
...
def test_get_psbt_purpose_from_storage(mocker):
    ...
    assert BroadcastTransactionService.get_psbt_purpose_from_storage('') is None
    
    mock_get_session = mocker.patch('src.data.service.broadcast_transaction_service.WalletDataService.get_session')
    mock_get_session.return_value = None
    assert BroadcastTransactionService.get_psbt_purpose_from_storage('psbt') is None
    ...
    assert BroadcastTransactionService.get_psbt_purpose_from_storage('my_psbt') == 'stored_purpose'
    
    # Exception suppression
    mock_cursor.execute.side_effect = Exception('boom')
    assert BroadcastTransactionService.get_psbt_purpose_from_storage('my_psbt') is None


def test_resolve_transfer_type(mocker):
    """Resolve transfer type delegates to parsed prefix, explicit string, or db layer."""
    assert BroadcastTransactionService.resolve_transfer_type('psbt', False, is_inflation=True) == 'inflation'
    assert BroadcastTransactionService.resolve_transfer_type('psbt', False, explicit_type='explicit') == 'explicit'
    assert BroadcastTransactionService.resolve_transfer_type('psbt', False) is None

    # Multisig active
    assert BroadcastTransactionService.resolve_transfer_type('psbt:send_asset:abc', True) == 'send_asset'
    
    mocker.patch('src.data.service.broadcast_transaction_service.BroadcastTransactionService.get_psbt_purpose_from_storage', return_value='db_purpose')
    assert BroadcastTransactionService.resolve_transfer_type('psbt_xyz', True) == 'db_purpose'


def test_can_enable_primary_and_reject_actions():
    """Business rule enforcement for primary 'Sign' / 'Broadcast' / 'Reject' features."""
    # Reject validations
    assert BroadcastTransactionService.can_enable_reject_action(False, False, True, True, False) is False
    assert BroadcastTransactionService.can_enable_reject_action(True, True, True, True, True) is True # watch only 
    assert BroadcastTransactionService.can_enable_reject_action(False, True, False, True, False) is True

    # Primary validations
    # Base fail
    assert BroadcastTransactionService.can_enable_primary_action('', True, False, False, False, False, 0, False, False, 5) is False
    
    # Broadcast Flow: valid selection
    assert BroadcastTransactionService.can_enable_primary_action('abcde', True, False, False, False, False, 0, True, False, 5) is True
    assert BroadcastTransactionService.can_enable_primary_action('abcde', True, False, False, False, False, -1, True, False, 5) is False
    
    # Broadcast Flow: multisig watch-only
    assert BroadcastTransactionService.can_enable_primary_action('abcde', True, True, True, True, True, 0, False, False, 5) is True
    
    # Signer Flow: Offline
    assert BroadcastTransactionService.can_enable_primary_action('psbt:purpose:abcde', False, True, False, False, False, 0, False, True, 5) is True
    assert BroadcastTransactionService.can_enable_primary_action('abcde', False, True, True, True, False, 0, False, False, 5) is True


def test_psbts_loaded_data():
    """Prepares structured responses for dropdown selectors."""
    assert BroadcastTransactionService.psbts_loaded_data([], True) == {'has_items': False}
    
    i1 = PsbtDraftItem(id='1', psbt='x', signed=True, purpose=None)
    assert BroadcastTransactionService.psbts_loaded_data([i1], True) == {'has_items': True, 'is_single': True, 'psbt': 'x'}
...
def test_receive_asset_model_for_signed_psbt(mocker):
    ...
    model = BroadcastTransactionService.receive_asset_model_for_signed_psbt('test')
...
def test_multisig_sign_loading_data(mocker):
    ...
    assert BroadcastTransactionService.multisig_sign_loading_data(None) == {'has_pending': False}
...
def test_should_enable_action_conditional_gates():
    ...
    # Action checks
    assert BroadcastTransactionService.should_enable_action(has_input=False, is_multisig=True, is_psbt_validated=True, pending_operation_present=True) is False
    assert BroadcastTransactionService.should_enable_action(has_input=True, is_multisig=False, is_psbt_validated=False, pending_operation_present=False) is True
    assert BroadcastTransactionService.should_enable_action(has_input=True, is_multisig=True, is_psbt_validated=True, pending_operation_present=True) is True
    assert BroadcastTransactionService.should_enable_action(has_input=True, is_multisig=True, is_psbt_validated=False, pending_operation_present=True) is False
...
def test_process_pending_operation_match_and_inspection(mocker):
    ...
    assert BroadcastTransactionService.process_pending_operation_match(None, None, False) is None
    
    mock_pending = MagicMock()
    mock_pending.is_initiator = True
    
    # Set operation instance to trick isinstance into passing if it doesn't strictly follow rgb-lib struct
    class MockOpReq:
        """Mock behavior struct"""
        def is_SEND_TO_REVIEW(self): return True
        def is_INFLATION_TO_REVIEW(self): return False
        def is_SEND_BTC_TO_REVIEW(self): return False
        def is_CREATE_UTXOS_TO_REVIEW(self): return False
        status = None
...
def test_resolve_inspection_context(mocker):
    ...
    mocker.patch('src.data.service.broadcast_transaction_service.BroadcastTransactionService.get_psbt_purpose_from_storage', return_value='inflate_asset')
    ...
    ctx = BroadcastTransactionService.resolve_inspection_context(op, None, 'psbt', False)
    assert ctx.is_inflation is True
    assert ctx.rgb_expected is True
    
    # Missing operation, fetches from purpose
    ctx2 = BroadcastTransactionService.resolve_inspection_context(None, 'send_asset', 'psbt', False)
    assert ctx2.is_inflation is False
    assert ctx2.rgb_expected is True
    
    # Fallback to storage
    ctx3 = BroadcastTransactionService.resolve_inspection_context(None, None, 'psbt', False)
...
def test_resolve_purpose_for_signing(mocker):
    ...
    mocker.patch('src.data.service.broadcast_transaction_service.BroadcastTransactionService.operation_transfer_type_key', return_value='op_key')
    ...
    assert BroadcastTransactionService.resolve_purpose_for_signing('parsed', None, '') == 'parsed'
    assert BroadcastTransactionService.resolve_purpose_for_signing(None, 'op_inst', '') == 'op_key'
    assert BroadcastTransactionService.resolve_purpose_for_signing(None, None, '') == 'db_key'


def test_prepare_state_contexts():
    """Validation of UI structural contexts and their property configurations."""
    # psbt text
    ctx1 = BroadcastTransactionService.prepare_psbt_text_changed_state('psbt:send_asset:abc', 'abc', 3)
