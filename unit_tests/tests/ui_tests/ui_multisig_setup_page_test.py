...
def test_multisig_without_password_redirects(vm, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.load_stylesheet', return_value='')
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_wallet_access_type', return_value=WalletAccessType.WATCH_ONLY)
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_wallet_network', return_value=NetworkEnumModel.TESTNET)
    mocker.patch('src.views.ui_multisig_setup_page.get_value', return_value=None)  # No password

    # Mock the page navigation signal
    mock_signal = MagicMock()
    mocker.patch('src.views.ui_multisig_setup_page.PageNavigationEventManager.get_instance')
...
def test_hardware_wallet_flow_with_saved_config(vm, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.load_stylesheet', return_value='')
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_wallet_access_type', return_value=WalletAccessType.WITH_PRIVATE_KEY)
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_wallet_network', return_value=NetworkEnumModel.TESTNET)
    mocker.patch('src.views.ui_multisig_setup_page.get_value', return_value='test_password')
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_multisig_config', return_value=(2, 3))
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_key_storage_type', return_value=KeyStorageType.HARDWARE_WALLET)
    mocker.patch('src.views.ui_multisig_setup_page.local_store.get_value', side_effect=['FP123', 'xpub_vanilla', 'xpub_colored'])
    mocker.patch.object(MultisigSetupPage, '_complete_threshold_confirmation_after_hw_connect')

    w = MultisigSetupPage(vm)
    w._complete_threshold_confirmation_after_hw_connect.assert_called_once_with(2, 3)
...
def test_restore_cosigner_inputs(vm, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.get_value', return_value='test_password')
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_multisig_config', return_value=(2, 3))
    mocker.patch('src.views.ui_multisig_setup_page.SettingRepository.get_cosigners', return_value=[
...
def test_on_watch_only_cosigner_string_changed_invalid(widget_watch_only, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.parse_cosigner_string', return_value=mocker.Mock(is_valid=False))
...
def test_update_continue_enabled_duplicate_cosigners(widget_with_privkey, mocker):
    ...
    mocker.patch.object(widget_with_privkey.cos_frame, 'isVisible', return_value=True)
    widget_with_privkey.cosigner_rows = [card1, card2]
    widget_with_privkey.required_signer_input.setText('2')
    widget_with_privkey.total_signer_input.setText('3') # n=3 means 2 cosigner cards
...
def test_export_cosigner_to_file(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.QFileDialog.getSaveFileName', return_value=('/tmp/test_export.txt', ''))
    mock_open = mocker.patch('builtins.open', mocker.mock_open())
    toast_success = mocker.patch('src.views.ui_multisig_setup_page.ToastManager.success')
    ...
    mock_open.assert_called_once_with('/tmp/test_export.txt', 'w', encoding='utf-8')
...
def test_import_cosigner_from_file(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.QFileDialog.getOpenFileName', return_value=('/tmp/test_import.txt', ''))
    mocker.patch('builtins.open', mocker.mock_open(read_data='imported_string'))
    toast_success = mocker.patch('src.views.ui_multisig_setup_page.ToastManager.success')
...
def test_on_watch_only_cosigner_string_changed_valid(widget_watch_only, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.parse_cosigner_string', return_value=mock_result)
...
def test_save_watch_only_review_fields(widget_watch_only, mocker):
    ...
    mock_save = mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.save_watch_only_data', return_value=True)
...
def test_populate_wallet_review_fields_not_watch_only(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.get_wallet_review_data', return_value=data)
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.truncate_text', side_effect=lambda x: x)
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.generate_cosigner_string', return_value='generated_str')
...
def test_on_cosigner_string_changed_valid(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.parse_cosigner_string', return_value=mock_result)
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.truncate_text', side_effect=lambda x: f'trunc_{x}')
...
def test_save_cosigners_data_valid(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.parse_cosigner_string', return_value=mocker.Mock(is_valid=True))
    mock_save = mocker.patch('src.views.ui_multisig_setup_page.MultisigSetupService.save_cosigners_data', return_value=True)
...
def test_export_cosigner_to_file_exception(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.QFileDialog.getSaveFileName', return_value=('/tmp/fail.txt', ''))
    mocker.patch('builtins.open', side_effect=Exception('Write Error'))
    toast_error = mocker.patch('src.views.ui_multisig_setup_page.ToastManager.error')
...
def test_import_cosigner_from_file_exception(widget_with_privkey, mocker):
    ...
    mocker.patch('src.views.ui_multisig_setup_page.QFileDialog.getOpenFileName', return_value=('/tmp/fail.txt', ''))
...
def test_restore_cosigner_inputs_partial_data(widget_watch_only, mocker):
    ...
    # Partial data
    widget_watch_only._restore_cosigner_inputs([{'index': 1, 'master_fingerprint': 'FP'}])
...
def test_update_continue_enabled_step_3_various_states(widget_with_privkey, mocker):
    ...
    widget_with_privkey._current_step = 3
    mocker.patch.object(widget_with_privkey.cos_frame, 'isVisible', return_value=True)
    widget_with_privkey.total_signer_input.setText('2')  # n=2 -> 1 cosigner card
