...
from src.utils.ledger_hw_client import (
    network_to_chain, _is_emulator_reachable, _get_hid_devices,
    _probe_device, enumerate_ledger_devices, create_ledger_client,
    _hardened, _get_bip44_coin_type, _build_multisig_policy,
    sign_psbt_with_ledger, _apply_signatures
)
...
def test_sign_psbt_with_ledger_singlesig(mock_psbt, mock_repo, mock_store, _mock_copy):
    ...
    mock_client.get_extended_pubkey.assert_called_with(path="m/86'/0'/5'", display=False)
...
def test_apply_signatures():
    ...
    # Tap script sig
    sig1 = PartialSignature(signature=b'sig1', pubkey=b'pub1', tapleaf_hash=b'hash')
    # Tap key sig (32 bytes pubkey)
    sig2 = PartialSignature(signature=b'sig2', pubkey=b'P' * 32, tapleaf_hash=None)
    # Partial sig (33 bytes legacy pubkey)
    sig3 = PartialSignature(signature=b'sig3', pubkey=b'P' * 33, tapleaf_hash=None)
    
    _apply_signatures(psbt, [(0, sig1), (0, sig2), (0, sig3), (99, sig1)]) # 99 is invalid index
