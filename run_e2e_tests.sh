...
APP4_NAME=$(grep '^APP4_NAME' accessible_constant.py | awk -F'=' '{print $2}' | tr -d ' "' | xargs)
...
FOURTH_WALLET_NAME="iris-wallet-vault_${APP4_NAME}-${VERSION}-x86_64.AppImage"
...
APP4_PATH="$APPLICATIONS_DIR/$FOURTH_WALLET_NAME"
...
build_applications() {
    ...
    echo "Building fourth wallet..."
    build-iris-wallet --network regtest --distribution appimage --app-name "${APP4_NAME}" &
    wait $!
    move_application "$FOURTH_WALLET_NAME"
...
ensure_applications_exist() {
    ...
    if [[ ! -f "$APP1_PATH" || ! -f "$APP2_PATH" || ! -f "$APP3_PATH" || ! -f "$APP4_PATH" ]]; then
    ...
    if [[ "$WALLET_VARIANT_SPECIFIED" == true ]]; then
        ...
        if [[ "$RUN_ALL" == true ]]; then
            ...
            for test_file in "${TEST_FILES[@]}"; do
                ...
                if ! pytest -s "$test_file" --timeout=3600 --alluredir="$results_dir" --wallet-variant "$SPECIFIED_WALLET_VARIANT" ${PYTEST_EXTRA_ARGS[@]:+"${PYTEST_EXTRA_ARGS[@]}"}; then
            ...
            if ! pytest -s "$TESTS_DIR/$TEST_FILE" --timeout=3600 --alluredir="$results_dir" --wallet-variant "$SPECIFIED_WALLET_VARIANT" ${PYTEST_EXTRA_ARGS[@]:+"${PYTEST_EXTRA_ARGS[@]}"}; then
        ...
        for variant in "${DEFAULT_WALLET_VARIANTS[@]}"; do
            ...
            if [[ "$RUN_ALL" == true ]]; then
                ...
                for test_file in "${TEST_FILES[@]}"; do
                    ...
                    if ! pytest -s "$test_file" --timeout=3600 --alluredir="$results_dir" --wallet-variant "$variant" ${PYTEST_EXTRA_ARGS[@]:+"${PYTEST_EXTRA_ARGS[@]}"}; then
                ...
                if ! pytest -s "$TESTS_DIR/$TEST_FILE" --timeout=3600 --alluredir="$results_dir" --wallet-variant "$variant" ${PYTEST_EXTRA_ARGS[@]:+"${PYTEST_EXTRA_ARGS[@]}"}; then
