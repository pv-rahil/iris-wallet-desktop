...
def _probe_device(transport: TransportClient) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        # Enforce RGB or RGB Test app
        if 'RGB' not in app_name:
            transport.stop()
            return None, None
...
def _build_multisig_policy(desc_str: str, wallet_name: str) -> WalletPolicy:
    ...
    match = re.search(
        r'tr\(([^,]+),\s*multi_a\((\d+),(.*?)\)\)', desc_str, re.DOTALL,
    )
    if not match:
    ...
    keys = [strip_wildcard(internal_key)] + [
        strip_wildcard(k)
        for k in participants.split(',') if k.strip()
    ]
...
def sign_psbt_with_ledger(
    ...
    # Parse original PSBT (keep as v0 if it is v0)
    psbt = PSBT()
    ...
    logger.debug(
        ...
        policy.descriptor_template, len(policy.keys_info),
    )
...
def _apply_signatures(psbt: Any, partial_sigs: list) -> None:
    ...
    for idx, sig_obj in partial_sigs:
        ...
        if sig_obj.tapleaf_hash is not None:
            inp.tap_script_sigs[(
                sig_obj.pubkey, sig_obj.tapleaf_hash,
            )] = sig_obj.signature
